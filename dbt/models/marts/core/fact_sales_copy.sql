...
SELECT vin, customer_id, model, invoice_date, price, price_class, periode,
    customer_name, customer_type, is_duplicate_suspect, created_at,
    CURRENT_TIMESTAMP() AS dbt_loaded_at
FROM {{ ref('int_sales_enriched') }} WHERE is_canonical = TRUE AND invoice_date IS NOT NULLfact_after_sales